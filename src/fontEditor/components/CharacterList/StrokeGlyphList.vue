<script setup lang="ts">
	...
	import { executeScript, addGlyphTemplate, clearGlyphRenderList, stroke_glyphs, editGlyph } from '../../stores/glyph'
	...
	const renderPreviewCanvas = () => {
		...
		const render = () => {
			if (i >= stroke_glyphs.value.length) return
			const glyph = stroke_glyphs.value[i]
			if (!glyph._o) {
				executeScript(glyph)
			}
	...
	onMounted(async () => {
		...
		emitter.on('renderStrokeGlyphPreviewCanvasByUUIDOnEditing', async (uuid: string) => {
			if (timerMap.get(uuid)) {
				clearTimeout(timerMap.get(uuid))
				timerMap.set(uuid, null)
			}
...
<template>
	<section class="stroke-glyph-list-wrapper">
		<el-scrollbar>
			<main class="list" id="stroke-glyph-render-list">
			</main>
		</el-scrollbar>
	</section>
</template>
