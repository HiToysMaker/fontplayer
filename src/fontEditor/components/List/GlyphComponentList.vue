<script setup lang="ts">
  ...
  const selectComponent = (e: MouseEvent, uuid: string) => {
    ...
    if (component.type === 'glyph' && component.value.components && component.value.components.length) {
      ...
      if (hasGlyph && hasSelected) {
        if (!editGlyph.value.selectedComponentsTree) {
          editGlyph.value.selectedComponentsTree = []
        }
        editGlyph.value.selectedComponentsTree.push(uuid)
  ...
  const onDragOver = (e: DragEvent, uuid: string) => {
    e.preventDefault()
    const list = orderedListForCurrentGlyph.value
  ...
  const onDrop = (e: DragEvent, uuid: string) => {
    const list: Array<{ type: string, uuid: string }> = R.clone(orderedListForCurrentGlyph.value)
    ...
    setOrderedListForCurrentGlyph(list)
  }
