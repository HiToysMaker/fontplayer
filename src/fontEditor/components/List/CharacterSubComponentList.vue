<script setup lang="ts">
  ...
  const selectComponent = (e: MouseEvent, uuid: string) => {
    if (!editCharacterFile.value.selectedComponentsTree || editCharacterFile.value.selectedComponentsTree.length < 2) return
    editCharacterFile.value.selectedComponentsTree[editCharacterFile.value.selectedComponentsTree.length - 1] = uuid
    const component = selectedItemByUUID(SubComponentsRoot.value.value.components, uuid)
  ...
  const remove = (uuid: string) => {
    ...
    if (selectedSubComponent.value === uuid) {
      let tree = editCharacterFile.value.selectedComponentsTree
  ...
  const onRootClick = () => {
    ...
    editCharacterFile.value.selectedComponentsTree = tree
  }
...
<template>
  <div class="list-wrapper">
    <div
      ...
      @pointerdown="onRootClick"
    >
      <el-icon class="back-icon" @pointerdown="onBackClick"><ArrowLeftBold /></el-icon>
      <div
