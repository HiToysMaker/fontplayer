...
// 横折2的骨架转骨骼函数
export const skeletonToBones_heng_zhe2 = (skeleton: any): any[] => {
  const bones: any[] = [];
  const { heng_start, heng_end, zhe_start, zhe_end } = skeleton;
...
const instanceBasicGlyph_heng_zhe2 = (plainGlyph: ICustomGlyph) => {
...
const bindSkeletonGlyph_heng_zhe2 = (plainGlyph: ICustomGlyph) => {
  if (!plainGlyph._o) {
    instanceBasicGlyph_heng_zhe2(plainGlyph)
  }
...
const updateSkeletonListener_before_bind_heng_zhe2 = (glyph: CustomGlyph) => {
...
const updateSkeletonListener_after_bind_heng_zhe2 = (glyph: CustomGlyph) => {
  const getJointsMap = (data) => {
    const { draggingJoint, deltaX, deltaY } = data
    const jointsMap = Object.assign({}, glyph.tempData)
    switch (draggingJoint.name) {
