...
// 竖弯的骨架转骨骼函数
export const skeletonToBones_shu_wan = (skeleton: any): any[] => {
  const bones: any[] = [];
  const { shu_start, shu_end, wan_start, wan_end } = skeleton;
...
const bindSkeletonGlyph_shu_wan = (plainGlyph: ICustomGlyph) => {
  if (!plainGlyph._o) {
    instanceBasicGlyph_shu_wan(plainGlyph)
  }
...
const updateSkeletonListener_before_bind_shu_wan = (glyph: CustomGlyph) => {
...
const updateSkeletonListener_after_bind_shu_wan = (glyph: CustomGlyph) => {
  const getJointsMap = (data) => {
    const { draggingJoint, deltaX, deltaY } = data
    const jointsMap = Object.assign({}, glyph.tempData)
    switch (draggingJoint.name) {
