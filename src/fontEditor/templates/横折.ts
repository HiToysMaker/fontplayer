...
// 横折的骨架转骨骼函数
export const skeletonToBones_heng_zhe = (skeleton: any): any[] => {
  const bones: any[] = [];
...
const instanceBasicGlyph_heng_zhe = (plainGlyph: ICustomGlyph) => {
...
const bindSkeletonGlyph_heng_zhe = (plainGlyph: ICustomGlyph) => {
  if (!plainGlyph._o) {
    instanceBasicGlyph_heng_zhe(plainGlyph)
  }
...
const updateSkeletonListener_before_bind_heng_zhe = (glyph: CustomGlyph) => {
...
const updateSkeletonListener_after_bind_heng_zhe = (glyph: CustomGlyph) => {
  const getJointsMap = (data) => {
    const { draggingJoint, deltaX, deltaY } = data
    const jointsMap = Object.assign({}, glyph.tempData)
    switch (draggingJoint.name) {
