...
export const skeletonToBones_heng_zhe_zhe_wan_gou = (skeleton: any): any[] => {
  ...
  for (let i = 0; i < gouSegments; i++) {
    ...
    if (i === 0) {
      ...
      bones[hengSegments + zhe1Segments + zhe2Segments + wanSegments - 1].children.push(bone.id);
    } else {
      bone.parent = `gou_segment_${i - 1}`;
      bones[hengSegments + zhe1Segments + zhe2Segments + wanSegments + i - 1].children.push(bone.id);
    }
...
const instanceBasicGlyph_heng_zhe_zhe_wan_gou = (plainGlyph: ICustomGlyph) => {
...
const bindSkeletonGlyph_heng_zhe_zhe_wan_gou = (plainGlyph: ICustomGlyph) => {
  if (!plainGlyph._o) {
    instanceBasicGlyph_heng_zhe_zhe_wan_gou(plainGlyph)
  }
...
const updateSkeletonListener_before_bind_heng_zhe_zhe_wan_gou = (glyph: CustomGlyph) => {
...
const updateSkeletonListener_after_bind_heng_zhe_zhe_wan_gou = (glyph: CustomGlyph) => {
  const getJointsMap = (data) => {
    const { draggingJoint, deltaX, deltaY } = data
    const jointsMap = Object.assign({}, glyph.tempData)
    switch (draggingJoint.name) {
