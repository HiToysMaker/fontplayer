...
// 竖折折钩的骨架转骨骼函数
export const skeletonToBones_shu_zhe_zhe_gou = (skeleton: any): any[] => {
  const bones: any[] = [];
  const { shu_start, shu_end, zhe1_start, zhe1_end, zhe2_start, zhe2_end, gou_start, gou_end } = skeleton;
  ...
  for (let i = 0; i < zhe2Segments; i++) {
    ...
    if (i === 0) {
      ...
      bones[shuSegments + zhe1Segments - 1].children.push(bone.id);
    } else {
  ...
  for (let i = 0; i < gouSegments; i++) {
    ...
    if (i === 0) {
      ...
      bones[shuSegments + zhe1Segments + zhe2Segments - 1].children.push(bone.id);
    } else {
      bone.parent = `gou_segment_${i - 1}`;
      bones[shuSegments + zhe1Segments + zhe2Segments + i - 1].children.push(bone.id);
    }
...
const instanceBasicGlyph_shu_zhe_zhe_gou = (plainGlyph: ICustomGlyph) => {
...
const bindSkeletonGlyph_shu_zhe_zhe_gou = (plainGlyph: ICustomGlyph) => {
  if (!plainGlyph._o) {
    instanceBasicGlyph_shu_zhe_zhe_gou(plainGlyph)
  }
...
const updateSkeletonListener_before_bind_shu_zhe_zhe_gou = (glyph: CustomGlyph) => {
...
const updateSkeletonListener_after_bind_shu_zhe_zhe_gou = (glyph: CustomGlyph) => {
  const getJointsMap = (data) => {
    const { draggingJoint, deltaX, deltaY } = data
    const jointsMap = Object.assign({}, glyph.tempData)
    switch (draggingJoint.name) {
