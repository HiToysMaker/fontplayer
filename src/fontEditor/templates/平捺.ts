...
// 平捺的骨架转骨骼函数
export const skeletonToBones_ping_na = (skeleton: any): any[] => {
  const bones: any[] = [];
...
const instanceBasicGlyph_ping_na = (plainGlyph: ICustomGlyph) => {
...
const bindSkeletonGlyph_ping_na = (plainGlyph: ICustomGlyph) => {
  if (!plainGlyph._o) {
    instanceBasicGlyph_ping_na(plainGlyph)
  }
...
const updateSkeletonListener_before_bind_ping_na = (glyph: CustomGlyph) => {
...
const updateSkeletonListener_after_bind_ping_na = (glyph: CustomGlyph) => {
  const getJointsMap = (data) => {
    const { draggingJoint, deltaX, deltaY } = data
    const jointsMap = Object.assign({}, glyph.tempData)
    switch (draggingJoint.name) {
