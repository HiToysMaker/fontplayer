...
// 撇的骨架转骨骼函数
export const skeletonToBones_pie = (skeleton: any): any[] => {
  const bones: any[] = [];
...
const instanceBasicGlyph_pie = (plainGlyph: ICustomGlyph) => {
...
const bindSkeletonGlyph_pie = (plainGlyph: ICustomGlyph) => {
  if (!plainGlyph._o) {
    instanceBasicGlyph_pie(plainGlyph)
  }
...
const updateSkeletonListener_before_bind_pie = (glyph: CustomGlyph) => {
...
const updateSkeletonListener_after_bind_pie = (glyph: CustomGlyph) => {
  const getJointsMap = (data) => {
    const { draggingJoint, deltaX, deltaY } = data
    const jointsMap = Object.assign({}, glyph.tempData)
    switch (draggingJoint.name) {
...
export {
  instanceBasicGlyph_pie,
  bindSkeletonGlyph_pie,
  updateSkeletonListener_after_bind_pie,
  updateSkeletonListener_before_bind_pie,
}