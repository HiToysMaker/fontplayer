...
// 定义所有需要创建的笔画
const strokes = [
  '平捺', '挑捺', '撇点', '撇挑', '横撇弯钩', '斜钩', '竖折', '竖弯钩', 
  '竖弯', '竖挑', '竖撇', '横折挑', '横折2', '横折弯', '二横折', 
  '横折', '横折折弯钩', '横折弯钩', '横弯钩', '横折钩', '横钩', 
  '横撇', '横折折撇', '竖折折钩', '竖钩', '弯钩'
];

// 基础模板
const baseTemplate = `import { CustomGlyph } from "../programming/CustomGlyph";
...
const instanceBasicGlyph_{strokeName} = (plainGlyph: ICustomGlyph) => {
  ...
  const params = {
    ...
    bendDegree: Number(glyph.getParam('弯曲度')) + 30 * Number(glyph.getParam('弯曲程度') || 1),
  }
...
const bindSkeletonGlyph_{strokeName} = (plainGlyph: ICustomGlyph) => {
  if (!plainGlyph._o) {
    instanceBasicGlyph_{strokeName}(plainGlyph)
  }
...
const updateSkeletonListener_before_bind_{strokeName} = (glyph: CustomGlyph) => {
  ...
  glyph.onSkeletonDragEnd = (data) => {
    ...
    glyph.setParam('弯曲度', _params.bendDegree - 30 * Number(glyph.getParam('弯曲程度') || 1))
    glyph.tempData = null
  }
}

const updateSkeletonListener_after_bind_{strokeName} = (glyph: CustomGlyph) => {
  const getJointsMap = (data) => {
    const { draggingJoint, deltaX, deltaY } = data
    const jointsMap = Object.assign({}, glyph.tempData)
    switch (draggingJoint.name) {
...
export {
  instanceBasicGlyph_{strokeName},
  bindSkeletonGlyph_{strokeName},
  updateSkeletonListener_after_bind_{strokeName},
  updateSkeletonListener_before_bind_{strokeName},
}`;
...
strokes.forEach(stroke => {
  ...
  const content = baseTemplate.replace(/{strokeName}/g, strokeName);
  const filePath = path.join(templatesDir, `${stroke}.ts`);
  
  fs.writeFileSync(filePath, content);
  console.log(`Generated: ${stroke}.ts`);
});

console.log('All stroke templates generated successfully!');
