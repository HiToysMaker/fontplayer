...
const getComponents = (skeleton) => {
  ...
  const { corner: end_bottom_corner } = FP.getIntersection({
    type: 'line',
    start: end_left_data.tangent.end,
    end: end_left_data.tangent.start,
  }, {
    type: 'line',
    start: in_na_curves[in_na_curves.length - 1].end,
    end: FP.turnAngle(
      in_na_curves[in_na_curves.length - 1].end,
      out_na_curves[out_na_curves_final.length - 1].end,
      Math.PI / 3,
      end_length,
    )
  })

  const start_p0 = FP.getPointOnLine(in_tiao_start, in_tiao_end, start_radius)
  const start_p1 = in_tiao_start
  const start_p2 = out_tiao_start
  const start_p3 = FP.getPointOnLine(out_tiao_start, out_tiao_end, start_radius)

  const end_p0 = end_left_data.point
  const end_p1 = end_bottom_corner
  const end_p2 = in_na_curves[in_na_curves.length - 1].end
  const end_p8 = end_right_data.point
  const end_p6 = FP.goStraight(end_right_data.tangent.end, end_right_data.tangent.start, 0.35 * end_length)
  const end_p7 = FP.turnRight(end_p6, end_p8, d * 2)
  const end_p5 = FP.turnLeft(end_p8, end_p6, d * 2)
  const end_p4 = FP.getPointOnLine(end_p6, end_p2, 0.2 * end_length)
  const end_p3 = FP.goStraight(end_p5, end_p4, d * 2)
