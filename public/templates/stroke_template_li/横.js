...
const getComponents = (skeleton, global_params) => {
  ...
  const end_p6 = end
  const end_p5 = FP.turnAngleFromEnd(start, end, -(FP.degreeToRadius(90) - endRightAngle), _weight * 0.5)
  const end_p7 = FP.turnAngleFromEnd(start, end, FP.degreeToRadius(90) + endRightAngle, _weight * 0.6)
  const end_p4 = FP.turnRight(end_p7, end_p5, _weight * 0.65)
  const end_p3 = FP.turnRight(end_p7, end_p5, _weight * 1.1)
  const end_p3_p1_vector = FP.turnRight(end_p5, end_p3, 100)
  const { corner: end_p1 } = FP.getIntersection(
    { type: 'line', start: end_p3, end: end_p3_p1_vector },
    { type: 'line', start: in_heng_start, end: in_heng_end }
  )
  const end_p2 = FP.getPointOnLine(end_p3, end_p1, FP.distance(end_p3, end_p1) * 0.5)
  const end_p0 = FP.getPointOnLine(end_p1, in_turn_p2, FP.distance(end_p1, in_turn_p2) * 0.5)
  const end_p9 = FP.turnLeft(end_p5, end_p7, 0.35 * _weight)
  const end_p8 = FP.getPointOnLine(end_p7, end_p9, FP.distance(end_p7, end_p9) * 0.5)
  const end_p10 = FP.getPointOnLine(end_p9, out_turn_p2, FP.distance(end_p9, out_turn_p2) * 0.5)
  ...
  const end_p9_joint = new FP.Joint('end_p9_joint', end_p9)
  const end_p10_joint = new FP.Joint('end_p10_joint', end_p10)
  ...
  glyph.addJoint(end_p9_joint)
  glyph.addJoint(end_p10_joint)
