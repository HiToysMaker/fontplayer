...
const getComponents = (skeleton) => {
  ...
  const start_top_vector_end = FP.turnAngleFromStart(pie_start, in_pie_curves_final[0].start, FP.degreeToRadius(-15), start_length)
  ...
  // 绘制左侧（外侧）轮廓
  pen.lineTo(start_left_data.final_curves[0].start.x, start_left_data.final_curves[0].start.y)
  for (let i = 0; i < start_left_data.final_curves.length; i++) {
    const curve = start_left_data.final_curves[i]
    pen.bezierTo(curve.control1.x, curve.control1.y, curve.control2.x, curve.control2.y, curve.end.x, curve.end.y)
  }
  pen.lineTo(out_corner_pie_tiao.x, out_corner_pie_tiao.y)
  pen.lineTo(in_tiao_curves[0].start.x, in_tiao_curves[0].start.y)
