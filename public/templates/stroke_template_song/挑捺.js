...
const getComponents = (skeleton) => {
  ...
  const end_bottom_vector_end = FP.turnAngleFromEnd(na_end, in_na_curves[in_na_curves.length - 1].end, FP.degreeToRadius(-60), end_length)
  let { corner: end_p0 } = FP.getIntersection(
  ...
  end_p0 = FP.goStraight(in_na_curves[in_na_curves.length - 1].end, end_p0, 0.3 * end_length)
  const end_p1 = na_end
  const end_p2 = out_na_curves[out_na_curves.length - 1].end
