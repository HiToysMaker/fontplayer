...
const getComponents = (skeleton, global_params) => {
  ...
  const radius = 10
  const _heng_weight = weight / stress_ratio
  ...
  // out指右侧（外侧）轮廓线
  // in指左侧（内侧）轮廓线
  const { out_heng_start, out_heng_end, in_heng_start, in_heng_end } = FP.getLineContours('heng', { heng_start, heng_end }, _heng_weight)
  ...
  const end_p0 = in_corner_heng_gou
  const end_p1 = in_gou_end
  const end_p3 = out_corner_heng_gou
  const end_p4 = out_corner_heng_gou_1
  const end_p5 = out_corner_heng_gou_2
  const end_p1_p2_vector = FP.turnAngleFromStart(end_p1, end_p3, turn_angle_1, 100)
  const end_p3_p2_vector = FP.turnAngleFromStart(end_p3, end_p1, -turn_angle_2, 100)
  const { corner: end_p2 } = FP.getIntersection(
    { type: 'line', start: end_p0, end: end_p1_p2_vector },
    { type: 'line', start: end_p3, end: end_p3_p2_vector },
  )
  ...
  const end_p4_radius_after = FP.getPointOnLine(end_p4, end_p5, radius)
  ...
  pen.quadraticBezierTo(end_p2.x, end_p2.y, end_p3_radius_before.x, end_p3_radius_before.y)
