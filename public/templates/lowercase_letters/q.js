...
const computeParamsByJoints = (jointsMap) => {
  ...
  const h2 = range(skeleton_9.y - skeleton_10.y, h2_range)
  const h3 = range(skeleton_1.y - skeleton_7.y, h3_range)
  const w1 = range(skeleton_1.x - skeleton_7.x, w1_range)
  const w2 = range(skeleton_10.x - skeleton_9.x, w2_range)
  return {
    h1,
    h2,
    h3,
...
const updateGlyphByParams = (params, global_params) => {
  ...
  const skeleton_6 = new FP.Joint('skeleton_6', {
    x: skeleton_5.x,
    y: skeleton_5.y + (h1 - h3 - weight / 2) / 2,
  })
  const skeleton_7 = new FP.Joint('skeleton_7', {
    x: skeleton_5.x,
    y: skeleton_5.y + (h1 - h3 - weight / 2),
  })
