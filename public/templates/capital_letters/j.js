...
const getComponents = (skeleton, global_params) => {
  ...
  const stroke1_start_serif_p4 = FP.getIntersection({
    ...
    type: 'line',
    start: in_stroke1_curves[0].start,
    end: in_stroke1_curves[0].control1,
  }).corner
  const stroke1_start_serif_p5 = FP.getIntersection({
    ...
    type: 'line',
    start: out_stroke1_curves[0].start,
    end: out_stroke1_curves[0].control1,
  }).corner
  const stroke1_start_serif_p6 = FP.goStraight(in_stroke1_curves[0].start, stroke1_start_serif_p4, serif_h1)
  const stroke1_start_serif_p7 = FP.goStraight(out_stroke1_curves[0].start, stroke1_start_serif_p5, serif_h1)
  ...
  const { final_curves: in_stroke1_curves_final } = FP.getRadiusPointsOnCurve(FP.getCurvesPoints(in_stroke1_curves), serif_h1 + serif_h2)
  const { final_curves: out_stroke1_curves_final } = FP.getRadiusPointsOnCurve(FP.getCurvesPoints(out_stroke1_curves), serif_h1 + serif_h2)
  ...
  if (serifType === 0) {
    ...
    for (let i = 0; i < in_stroke1_curves_final.length; i++) {
      const curve = in_stroke1_curves_final[i]
      pen1.bezierTo(curve.control1.x, curve.control1.y, curve.control2.x, curve.control2.y, curve.end.x, curve.end.y)
    }
    pen1.lineTo(out_stroke1_curves_final[out_stroke1_curves_final.length - 1].end.x, out_stroke1_curves_final[out_stroke1_curves_final.length - 1].end.y)
    for (let i = out_stroke1_curves_final.length - 1; i >= 0; i--) {
      const curve = out_stroke1_curves_final[i]
      pen1.bezierTo(curve.control2.x, curve.control2.y, curve.control1.x, curve.control1.y, curve.start.x, curve.start.y)
    }
    pen1.bezierTo(
